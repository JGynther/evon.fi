  ...
  return (
    <PageWrapper>
      ...
      <Section>
        ...
        <Title>Sitova osakeantisitoumus</Title>
        <Prose large>
          Täytä oheinen lomake erittäin huolellisesti. Täyttämällä lomakkeen
          suostut{" "}
          <Link href="/tietosuojaseloste.pdf" passHref>
            <a
              target="_blank"
              className="transition text-indigo-500 hover:text-indigo-700"
            >
              {"tietosuojaselosteemme"}
            </a>
          </Link>{" "}
          mukaiseen tietojesi käsittelyyn. Täyttämällä lomakkeen myös vakuutat
          tietojesi olevan oikein ja ajantasaisia.
        </Prose>
      ...
      <Formik
        ...
        validationSchema={Yup.object({
          ...
          acceptedterms: Yup.boolean()
            ...
            .required("Pakollinen"),
          streetaddress: Yup.string().required("Pakollinen"),
          postalcode: Yup.string().required("Pakollinen"),
          city: Yup.string().required("Pakollinen"),
          stock: Yup.number()
            .typeError("Lukumäärän tulee olla numero")
            .min(1, "Osakkeita tulee merkitä vähintään yksi")
            .max(
              100000,
              "Jos haluat merkitä yli 100 000 osaketta, ota yhteyttä"
            )
            .required("Pakollinen"),
          captcha: Yup.string().typeError("Pakollinen").required("Pakollinen"),
        })}
      >
        {({ isSubmitting, setFieldValue, errors }) => (
          <Form>
            <InputFieldWithFeedback
              ...
              id="name"
              name="name"
              placeholder="Matti Mikko Meikäläinen"
              helpText="Muodossa kaikki etunimet sukunimi"
              type="text"
              autoFocus
            />
            <InputFieldWithFeedback
              ...
              id="email"
              name="email"
              placeholder="matti@meikäläinen.fi"
              helpText=""
              type="email"
            />
            <InputFieldWithFeedback
              ...
              helpText="Puhelinnumero ilman maakoodia, muotoa 0XX 1234567"
              type="tel"
            />
...
function CaptchaElement({ setFieldValue }) {
  return (
    <div className="flex flex-wrap items-center gap-5">
      <ReCAPTCHA
        sitekey="6LddGPwbAAAAAOIcVYk9XtQ3APmdEeU7eAOfl30q"
        onChange={(value) => {
      ...
      <ErrorMessage
        name="captcha"
        component="p"
        className="text-red-700 text-lg tracking-wider"
      />
    </div>
  );
}
  ...
  return (
    <div className="flex flex-wrap gap-5 items-center border-2 border-gray-700 rounded px-4 md:px-10 py-3 md:py-5">
      <div>
        ...
        <Prose large>
          Kuinka monta osaketta olet merkitsemässä? Osakkeita on merkittävä
          vähintään yksi (1) kappale. Osakkeiden merkintähinta on yhtiökokouksen
          päätöksen mukaisesti 0,30 EUR kappale. Osakkeita tarjotaan
          merkittäväksi yhteensä miljoona (1 000 000) kappaletta.
        </Prose>
        <Prose large>
          Tällä lomakkeella voi merkitä maksimissaan 100 000 osaketta. Jos
          haluat merkitä enemmän,{" "}
          <Link href="/contact" passHref>
            <a className="text-indigo-500 hover:text-indigo-700 transition">
              ota suoraan yhteyttä.
            </a>
          </Link>
        </Prose>
        <Prose large>
          Päätös merkitä osakkeita on sitova. Tutustu riskeihin ennen
          sijoittamista.
        </Prose>
      </div>
    ...
    <div className="grid items-center my-12">
      <span className="flex items-center gap-4 mb-4 md:mb-0">
        ...
        <span className="grid md:flex items-center gap-3">
          <p className="tracking-wider text-white text-opacity-80 text-sm md:text-base">
            Olen lukenut, ymmärtänyt ja hyväksyn{" "}
          </p>
...
function InputFieldWithFeedback({
  label,
  helpText,
  noGap,
  fill,
    ...
    <div className={`${noGap ? "" : "my-6"} ${fill ? "flex-grow" : ""}`}>
      <div className="grid">
        <div className="flex flex-wrap justify-between items-center">
          ...
          <p
            className={`transition text-lg tracking-wider whitespace-normal ${
              meta.error ? "text-red-700" : "text-green-500 text-2xl"
            }`}
          >
            {showFeedback ? (meta.error ? meta.error : "✓") : null}
          </p>
        </div>
        <input
          {...props}
          {...field}
          onFocus={handleFocus}
          className={`py-2 px-3 my-1 rounded bg-gray-800 shadow tracking-wider outline-none ring ring-transparent focus:ring-blue-500 ${
            showFeedback
              ? meta.error
                ? "ring-red-700"
                : "ring-green-500"
...
function SubmitResetButtons({ isSubmitting, errors }) {
  return (
    <div className="flex flex-wrap justify-center md:justify-start items-center gap-7 md:gap-10 mt-5">
      ...
      <button
        type="reset"
        className="py-2 px-5 bg-gray-800 hover:bg-gray-700 transition rounded tracking-wider border-2 border-gray-700"
      >
        Nollaa lomake
      </button>
    </div>
